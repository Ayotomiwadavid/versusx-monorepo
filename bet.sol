...
contract ESkillzStraightBet is MetaKeepLambda {
    ...
    function CreateSPGame(
        address _sender,
        uint256 amount
    ) external onlySportContract {
        require(
            minBetAmounts <= amount,
            "Bet amounts must be bigger than minBetAmounts"
        );
    ...
    function SetSPGameResult(
        ...
        (
            ...
            uint256 amountToESkillz
        ) = getAmountsSPGameToDistribute(GameID);
        require(
            gamebetting[GameID][0].player == winner,
            "Other Players can not access."
        );
        require(
            gamebetting[GameID][0].gameType == 0,
            "You can set the SP game only"
        );
        require(
            ISport(sport).balanceOf(address(this)) >=
                gamebetting[GameID][0].amount,
            "Dport Balance of Bet contract is not enough."
        );
        ISport(sport).mintMore(
            winner,
            (gamebetting[GameID][0].amount * result) / 100
        );
        ISport(sport).transfer(winner, amountToWinner);
        ISport(sport).transfer(feeReceiver, amountToESkillz);
        if (
            totalAmountsOfDay[block.timestamp] >=
            (gamebetting[GameID][0].amount)
        ) {
            totalAmountsOfDay[block.timestamp] -= (
                gamebetting[GameID][0].amount
            ...
            if ((block.timestamp - startTimeStamp) / 1 days > 0) {
                totalAmountsOfDay[block.timestamp - 1] -= (
                    gamebetting[GameID][0].amount
                );
            }
        }
    ...
    function SetSPGameResultByToken(
        address winner,
        uint256 GameID,
        uint256 result
    ) external {
        ...
        require(
            gamebetting[GameID][0].betType == 1,
            "You can set the SP game only"
        );

        ISport(sport).mintMore(
            winner,
            amountToWinner + (gamebetting[GameID][0].amount * result) / 100
        );
        delete (gamebetting[GameID]);
    }
    ...
    function JoinMPGame(
        ...
        require(
            gamebetting[gameID][0].player != _sender,
            "Same Players can not join."
        );
        require(
            amount == gamebetting[gameID][0].amount,
            "Your bet amount must equals create amount"
        );
        require(
            gamebetting[gameID][0].gameType == 1,
            "You can join the MP game only"
        );
        totalAmountsOfDay[block.timestamp] += amount;
        ISport(sport).transferFrom(_sender, address(this), amount);
    ...
    function SetMPGameResult(address winner, uint256 GameID) external {
        (
            uint256 amountToWinner,
            uint256 amountToESkillz
        ) = getAmountsMPGameToDistribute(GameID);
        require(
            gamebetting[GameID][0].gameType == 1 &&
                gamebetting[GameID][1].gameType == 1,
            "You can set the MP game only"
        );
        require(
            winner == gamebetting[GameID][0].player ||
                winner == gamebetting[GameID][1].player,
            "msg sender must includes in this game"
        );
        require(
            ISport(sport).balanceOf(address(this)) >=
                gamebetting[GameID][0].amount * 2,
            "Dport Balance of Bet contract is not enough."
        );

        if (winner == gamebetting[GameID][0].player) {
            ISport(sport).transfer(
                gamebetting[GameID][0].player,
                amountToWinner
            );
        } else {
            ISport(sport).transfer(
                gamebetting[GameID][1].player,
                amountToWinner
            );
        }
        ISport(sport).transfer(feeReceiver, amountToESkillz);
        if (
            totalAmountsOfDay[(block.timestamp - startTimeStamp) / 1 days] >=
            (amountToWinner + amountToESkillz)
        ) {
            totalAmountsOfDay[
            ...
            if ((block.timestamp - startTimeStamp) / 1 days > 0) {
                totalAmountsOfDay[
                    (block.timestamp - startTimeStamp) / 1 days - 1
                ] -= (amountToWinner + amountToESkillz);
            }
        }
        delete (gamebetting[GameID]);
    ...
    function getAmountsSPGameToDistribute(
        uint256 game
    ) private view returns (uint256, uint256) {
        uint256 amountToESkillz = (gamebetting[game][0].amount * eskillz_fee) /
            10000;
        uint256 amountToWinner = gamebetting[game][0].amount - amountToESkillz;
        return (amountToWinner, amountToESkillz);
    }

    function getAmountsMPGameToDistribute(
        uint256 game
    ) private view returns (uint256, uint256) {
        uint256 amountToESkillz = (gamebetting[game][0].amount * eskillz_fee) /
            10000;
        uint256 amountToWinner = 2 *
            gamebetting[game][0].amount -
            amountToESkillz;
        return (amountToWinner, amountToESkillz);
    }
    ...
    function getAvailableAmountOfContract() external view returns (uint256) {
        if ((block.timestamp - startTimeStamp) / 1 days > 0) {
            return
                ISport(sport).balanceOf(address(this)) -
                totalAmountsOfDay[block.timestamp] -
                totalAmountsOfDay[block.timestamp - 86400];
        } else {
            return 0;
        }
    }
    ...
    function withdraw(address _feeReceiver, uint256 _amount) external {
        require(
            (block.timestamp - startTimeStamp) / 1 days > 0,
            "withdraw day should be bigger than start day + 1 day."
        );
        uint256 remainAmounts = totalAmountsOfDay[block.timestamp] +
            totalAmountsOfDay[block.timestamp - 86400];
        require(
            ISport(sport).balanceOf(address(this)) - _amount > remainAmounts,
            "Balance must be bigger than amount + bettingAmounts of today and yesterday."
        );
        require(
            feeReceiver == _feeReceiver || owner == _msgSender(),
            "msg sender must be feeReceiver or contract owner"
        );
    ...
    function withdrawAll(address _feeReceiver) external {
        ...
        uint256 remainAmounts = totalAmountsOfDay[block.timestamp] +
            totalAmountsOfDay[block.timestamp - 86400];
        require(
            ISport(sport).balanceOf(address(this)) > remainAmounts,
            "Balance must be bigger than amount + bettingAmounts of today and yesterday."
        );
        require(
            feeReceiver == _feeReceiver || owner == _msgSender(),
