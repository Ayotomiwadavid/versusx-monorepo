...
import { CCard, CCardBody, CCol, CCardHeader, CRow, CButton, CFormLabel, CFormInput, CFormSelect } from '@coreui/react'
  ...
  async function init() {
    ...
    for (var i = 0; i < keysBuf.length; i++) {
      if (typeof docSnap.data()[keysBuf[i]] === 'object') {
        items[keysBuf[i]] = dayjs(docSnap.data()[keysBuf[i]].seconds * 1000)
          .toISOString()
          .substring(0, 19)
        types[keysBuf[i]] = 2
      } else {
                ...
                <CRow key={key} class={myStyles}>
                  <CCol xs={2}>
                    <CFormInput
                      ...
                      onChange={(event) => {
                        updateName(key, event.target.value)
                      }}
                    />
                  </CCol>
                  {dataType[key] === 2 ? (
                    ...
                    <CCol xs={3}>
                      <CFormInput
                        ...
                        size="sm"
                        id="winpercent0"
                        placeholder={tournamentData[key]}
                        value={tournamentData[key]}
                        onChange={(event) => {
            ...
            {addType === 2 ? (
              ...
              <CCol xs={3}>
                <CFormInput
                  ...
                  value={addData}
                  onChange={(event) => {
                    if (Number(event.target.value) > 0) setAddData(event.target.value)
                  }}
                />
              </CCol>
            ) : (
