...
const getOwnNfts = async (nftaddress) => {
  try {
    let nftListData = await axios.get(`https://polygon-mainnet.g.alchemy.com/nft/v2/${process.env.REACT_APP_ALCHEMY_KEY}/getNFTs?owner=${process.env.REACT_APP_TREASURY_ADDRESS}&contractAddresses[]=${nftaddress}&withMetadata=true&pageSize=100`)
...
function HomePage() {
  ...
  const loadNfts = useCallback(async () => {
    ...
    console.log('=========')
    setNfts(items)
  }, [byAndSellSelected, curCollection])
  ...
  const getAllCollections = async () => {
    try {
      ...
      if (res.data.status === true) {
        let tempa = res.data.data.map((e, i) => {
          console.log(e)
          if (i === 0) {
            setCurCollection(e)
          }
  ...
  useEffect(() => {
    loadNfts()
  }, [byAndSellSelected, loadNfts])

  return (
    <CCard>
      <CCardHeader>NFTs</CCardHeader>
      <CCardBody>
          ...
          {collectionData.length > 0 && (
            ...
            >
              {collectionData.map((e, ci) => {
                return (
                  <MenuItem key={ci} style={{ display: 'grid' }} value={ci}>
                    {e.collection_name}
                  </MenuItem>
                )
              })}
