...
import { CFormTextarea, CSpinner, CFormSelect, CRow, CFormInput, CCard, CCardBody, CCardHeader, CButton, CCol } from '@coreui/react'
...
async function mintNft(_tokenUri, nftType, nftAddress, userAddress, quantity) {
  try {
    ...
    if (nftType === 'ERC721') {
      tx = userAddress === '' ? nftContract.methods.createToken(_tokenUri) : nftContract.methods.createTokenToUser(userAddress, _tokenUri)
    } else {
      tx = userAddress === '' ? nftContract.methods.createToken(_tokenUri, quantity) : nftContract.methods.createTokenToUser(userAddress, _tokenUri, quantity)
    }
      ...
      <CCard>
        ...
        <CCardBody>
          ...
          <CRow className="g-3">
            <CCol sm={3}>
              ...
              >
                {collectionData.map((e) => (
                  <option key={e.collection_address} value={e.collection_address}>
                    {e.collection_name}
                  </option>
                ))}
              </CFormSelect>
            ...
            <CCol sm={9}>
              <CButton color="danger" variant="outline" size="sm" style={{ marginTop: '4px' }}>
                {curNFTType}
              </CButton>
            </CCol>
          </CRow>
          <CRow className="g-3 mt-3">
            <CCol sm={6} style={{ display: 'flex', alignItems: 'center' }}>
              ...
              >
                ...
                {imgSrc === 'Pending' && <CSpinner color="primary" />}
                {imgSrc !== '' && imgSrc !== 'Pending' && (
                  <div>
                    {/* <span style={{ margin: '20px 0px 0px 0px' }}>{imgNameUpload}</span> */}
                    <img id="output" src={imgSrc} style={{ width: '100%' }} alt="output" />
                  </div>
                )}
              </div>
            </CCol>
