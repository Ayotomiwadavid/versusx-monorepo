...
const getOwnNfts = async (nftaddress, useraddress) => {
  try {
    let nftListData = await axios.get(`https://polygon-mainnet.g.alchemy.com/nft/v2/${process.env.REACT_APP_ALCHEMY_KEY}/getNFTs?owner=${useraddress}&contractAddresses[]=${nftaddress}&withMetadata=true&pageSize=100`);
    let nftList = nftListData.data.ownedNfts;

    const items = await Promise.all(nftList.map(async nftItem => {
      let nftMetadata = await axios.get(`https://polygon-mainnet.g.alchemy.com/nft/v2/${process.env.REACT_APP_ALCHEMY_KEY}/getNFTMetadata?contractAddress=${nftaddress}&tokenId=${ethers.BigNumber.from(nftItem["id"]["tokenId"]).toNumber().toString()}&refreshCache=true`).then(res=>res.data);
      // console.log("~~~~~~~~~~");
